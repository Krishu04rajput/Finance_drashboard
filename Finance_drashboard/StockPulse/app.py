...
def telegram_alert_ui():
    ...
    if connect:
        if not telegram_token or not chat_id:
            ...
            st.success("Telegram connected! You will receive alerts here.")
    if st.session_state.get("telegram_token") and st.session_state.get("telegram_chat_id"):
        st.info(f"Connected to Telegram chat ID: {st.session_state.telegram_chat_id}")
        test_message = st.text_input("Test Message", value="Hello from PROJECTX!")
        if st.button("Send Test Alert"):
            try:
                bot = Bot(token=st.session_state.telegram_token)
                bot.send_message(chat_id=st.session_state.telegram_chat_id, text=test_message)
...
def login_signup_ui():
    ...
    if submit:
        ...
        if auth_mode == "Sign Up":
            ...
            existing_username = db_session.query(db.User).filter(db.User.username == username).first()
            existing_email = db_session.query(db.User).filter(db.User.email == email).first()
            ...
            if profile_photo:
                ...
                photo_path = f"static/{username}_profile.png"
                with open(photo_path, "wb") as f:
                    f.write(profile_photo.read())
            user = db.User(username=username, email=email, password=hash_password(password), profile_photo=photo_path)
            db_session.add(user)
            db_session.commit()
...
# --- Require login/signup before dashboard ---
...
def telegram_alert_ui():
    st.header("Telegram Alerts Setup")
    st.markdown("Set up Telegram alerts to receive notifications for financial events.")
    telegram_token = st.text_input("Telegram Bot Token", type="password")
    chat_id = st.text_input("Your Telegram Chat ID")
    test_message = st.text_input("Test Message", value="Hello from PROJECTX!")
    if st.button("Send Test Alert"):
